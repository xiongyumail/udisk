...
esp_err_t apds9960_write_byte(apds9960_handle_t sensor, uint8_t reg_addr, uint8_t data)
{
    return apds9960_write(sensor, reg_addr, &data, 1);
}

esp_err_t apds9960_write(apds9960_handle_t sensor, uint8_t addr, uint8_t *buf, uint8_t len)
{
    apds9960_dev_t* sens = (apds9960_dev_t*) sensor;
    ...
    ret = i2c_master_cmd_begin(sens->bus, cmd, sens->timeout / portTICK_RATE_MS);
...
esp_err_t apds9960_read_byte(apds9960_handle_t sensor, uint8_t reg, uint8_t *data)
{
    return apds9960_read(sensor, reg, data, 1);
}

esp_err_t apds9960_read(apds9960_handle_t sensor, uint8_t reg_addr, uint8_t *buf, uint8_t len)
{
    apds9960_dev_t* sens = (apds9960_dev_t*) sensor;
    esp_err_t ret = ESP_FAIL;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
...
esp_err_t apds9960_get_deviceid(apds9960_handle_t sensor, uint8_t* deviceid)
{
    esp_err_t ret;
    uint8_t tmp;
    ret = apds9960_read_byte(sensor, APDS9960_WHO_AM_I_REG, &tmp);
    *deviceid = tmp;
    return ret;
}
...
{
    uint8_t tmp;
    if (apds9960_read_byte(sensor, APDS9960_MODE_ENABLE, &tmp) == ESP_FAIL) {
        return ESP_FAIL;
    }
    tmp |= mode;
    return apds9960_write_byte(sensor, APDS9960_MODE_ENABLE, tmp);
}
...
apds9960_mode_t apds9960_get_mode(apds9960_handle_t sensor)
{
    ...
    if (apds9960_read_byte(sensor, APDS9960_MODE_ENABLE, &value) == ESP_FAIL) {
        return ESP_FAIL;
    }
...
esp_err_t apds9960_enable_gesture_engine(apds9960_handle_t sensor, bool en)
{
    ...
    if (!en) {
        ...
        if (apds9960_write_byte(sensor, APDS9960_GCONF4,
                (sens->_gconf4_t.gien << 1) | sens->_gconf4_t.gmode) == ESP_FAIL) {
            return ESP_FAIL;
        }
    }
    sens->_enable_t.gen = en;
    ret = apds9960_write_byte(sensor, APDS9960_MODE_ENABLE,
        ...
        apds9960_ledboost_t boost)
{
    ...
    if (apds9960_write_byte(sensor, APDS9960_CONFIG2,
    ...
    return apds9960_write_byte(sensor, APDS9960_CONTROL,
...
{
    return apds9960_write_byte(sensor, APDS9960_WTIME, time);
}
...
esp_err_t apds9960_set_adc_integration_time(apds9960_handle_t sensor, uint16_t iTimeMS)
{
    ...
    return apds9960_write_byte(sensor, APDS9960_ATIME, (uint8_t) temp);
}
...
float apds9960_get_adc_integration_time(apds9960_handle_t sensor)
{
    ...
    if (apds9960_read_byte(sensor, APDS9960_ATIME, &val) == ESP_FAIL) {
        return ESP_FAIL;
    }
    temp = val;
...
{
    uint8_t val;
    if (apds9960_read_byte(sensor, APDS9960_CONTROL, &val) == ESP_FAIL) {
        return ESP_FAIL;
    }
...
esp_err_t apds9960_enable_proximity_engine(apds9960_handle_t sensor, bool en)
{
    ...
    return apds9960_write_byte(sensor, APDS9960_MODE_ENABLE,
            ((sens->_enable_t.gen << 6) | (sens->_enable_t.pien << 5) | (sens->_enable_t.aien << 4)
                    | (sens->_enable_t.wen << 3) | (sens->_enable_t.pen << 2) | (sens->_enable_t.aen << 1)
                    | sens->_enable_t.pon));
}
...
esp_err_t apds9960_set_proximity_pulse(apds9960_handle_t sensor, apds9960_ppulse_len_t pLen, uint8_t pulses)
{
    ...
    return apds9960_write_byte(sensor, APDS9960_PPULSE, (sens->_ppulse_t.pplen << 6) | sens->_ppulse_t.ppulse);
}
...
bool apds9960_color_data_ready(apds9960_handle_t sensor)
{
    uint8_t data;
    apds9960_dev_t* sens = (apds9960_dev_t*) sensor;
    apds9960_read_byte(sensor, APDS9960_STATUS, &data);
...
{
    uint8_t data[2] = { 0 };
    apds9960_read(sensor, APDS9960_CDATAL, data, 2);
    *c = (data[0] << 8) | data[1];
    apds9960_read(sensor, APDS9960_RDATAL, data, 2);
    *r = (data[0] << 8) | data[1];
    apds9960_read(sensor, APDS9960_GDATAL, data, 2);
    *g = (data[0] << 8) | data[1];
    apds9960_read(sensor, APDS9960_BDATAL, data, 2);
    *b = (data[0] << 8) | data[1];
    return ESP_OK;
}
...
{
    apds9960_write_byte(sensor, APDS9960_AILTL, low & 0xFF);
    apds9960_write_byte(sensor, APDS9960_AILTH, low >> 8);
    apds9960_write_byte(sensor, APDS9960_AIHTL, high & 0xFF);
    return apds9960_write_byte(sensor, APDS9960_AIHTH, high >> 8);
}
...
{
    uint8_t data;
    if (apds9960_read_byte(sensor, APDS9960_PDATA, &data)) {
        return ESP_FAIL;
    }
    return data;
}
...
{
    apds9960_dev_t* sens = (apds9960_dev_t*) sensor;
    apds9960_write_byte(sensor, APDS9960_PILT, low);
    apds9960_write_byte(sensor, APDS9960_PIHT, high);
    ...
    return apds9960_write_byte(sensor, APDS9960_PERS, (sens->_pers_t.ppers << 4) | sens->_pers_t.apers);
}
...
esp_err_t apds9960_set_gesture_dimensions(apds9960_handle_t sensor, uint8_t dims)
{
    ...
    return apds9960_write_byte(sensor, APDS9960_GCONF3, sens->_gconf3_t.gdims);
}
...
esp_err_t apds9960_set_light_intlow_threshold(apds9960_handle_t sensor, uint16_t threshold)
{
    ...
    /* Write low byte */
    if (apds9960_write_byte(sensor, APDS9960_AILTL, val_low) == ESP_FAIL) {
        return ESP_FAIL;
    }

    /* Write high byte */
    return apds9960_write_byte(sensor, APDS9960_AILTH, val_high);
}
...
esp_err_t apds9960_set_light_inthigh_threshold(apds9960_handle_t sensor, uint16_t threshold)
{
    ...
    /* Write low byte */
    if (apds9960_write_byte(sensor, APDS9960_AIHTL, val_low) == ESP_FAIL) {
        return ESP_FAIL;
    }

    /* Write high byte */
    return apds9960_write_byte(sensor, APDS9960_AIHTH, val_high);
}
...
esp_err_t apds9960_set_gesture_fifo_threshold(apds9960_handle_t sensor, uint8_t thresh)
{
    ...
    return apds9960_write_byte(sensor, APDS9960_GCONF1,
...
esp_err_t apds9960_set_gesture_waittime(apds9960_handle_t sensor, apds9960_gwtime_t time)
{
    ...
    if (apds9960_read_byte(sensor, APDS9960_GCONF2, &val) == ESP_FAIL) {
        return ESP_FAIL;
    }
    ...
    return apds9960_write_byte(sensor, APDS9960_GCONF2, val);
}
...
esp_err_t apds9960_set_gesture_gain(apds9960_handle_t sensor, apds9960_ggain_t gain)
{
    ...
    return apds9960_write_byte(sensor, APDS9960_GCONF2,
...
{
    esp_err_t ret;
    if (apds9960_write_byte(sensor, APDS9960_GPENTH, entthresh)) {
        return ESP_FAIL;
    }
    ret = apds9960_write_byte(sensor, APDS9960_GEXTH, exitthresh);
    return ret;
}
...
{
    apds9960_write_byte(sensor, APDS9960_GOFFSET_U, offset_up);
    apds9960_write_byte(sensor, APDS9960_GOFFSET_D, offset_down);
    apds9960_write_byte(sensor, APDS9960_GOFFSET_L, offset_left);
    apds9960_write_byte(sensor, APDS9960_GOFFSET_R, offset_right);
    return ESP_OK;
}
...
uint8_t apds9960_read_gesture(apds9960_handle_t sensor)
{
    ...
    while (1) {
        ...
        apds9960_read_byte(sensor, APDS9960_GFLVL, &toRead);
        apds9960_read(sensor, APDS9960_GFIFO_U, buf, toRead);
        bytesRead = toRead;
...
bool apds9960_gesture_valid(apds9960_handle_t sensor)
{
    uint8_t data;
    apds9960_dev_t* sens = (apds9960_dev_t*) sensor;
    apds9960_read_byte(sensor, APDS9960_GSTATUS, &data);
    sens->_gstatus_t.gfov = (data >> 1) & 0x01;
    sens->_gstatus_t.gvalid = data & 0x01;
...
esp_err_t apds9960_set_gesture_pulse(apds9960_handle_t sensor, apds9960_gpulselen_t gpulseLen, uint8_t pulses)
{
    ...
    return apds9960_write_byte(sensor, APDS9960_GPULSE, (sens->_gpulse_t.gplen << 6) | sens->_gpulse_t.gpulse);
}

esp_err_t apds9960_set_gesture_enter_thresh(apds9960_handle_t sensor, uint8_t threshold)
{
    esp_err_t ret;
    ret = apds9960_write_byte(sensor, APDS9960_GPENTH, threshold);
    return ret;
}
...
{
    esp_err_t ret;
    ret = apds9960_write_byte(sensor, APDS9960_GEXTH, threshold);
    return ret;
}

apds9960_handle_t apds9960_create(i2c_port_t bus, uint16_t dev_addr)
{
...
esp_err_t apds9960_delete(apds9960_handle_t sensor, bool del_bus)
{
    apds9960_dev_t* sens = (apds9960_dev_t*) sensor;
    if (del_bus) {
        i2c_driver_delete(sens->bus);
    }
    free(sens);
    return ESP_OK;
}
...
esp_err_t apds9960_gesture_init(apds9960_handle_t sensor)
{
    ...
    apds9960_set_gesture_gain(sensor, APDS9960_GGAIN_8X);
    apds9960_set_gesture_proximity_threshold(sensor, 10, 10);
    apds9960_reset_counts(sensor);
    ...
    apds9960_set_gesture_pulse(sensor, APDS9960_GPULSELEN_8US, 8);

    apds9960_enable_color_engine(sensor, true);
